...
if [ "$IP" != "$LAST_IP" ]; then
    echo "Current IP: $IP"
    RESULT=`curl -4 --silent "https://$USERNAME:$PASSWORD@$UPDATEURL/nic/update?hostname=$HOSTNAME&myip=$IP" | grep -o -E  "good|nochg|abuse|badauth|notfqdn|nohost|abuse|dnserr"`
